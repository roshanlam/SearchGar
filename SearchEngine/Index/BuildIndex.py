...
class BuildIndex:
	...
	genScore = lambda  self, term, document: self.tf[document][term] * self.idf[term]
	execute = lambda self: self.fullIndex()
	regIndex = lambda self: self.make_indices(self.file_to_terms)
	getUniques = lambda self: self.totalIndex.keys()
	#def execute(self):
